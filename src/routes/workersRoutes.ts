...
import { isUserExistsById, isUsernameAlreadyExist } from '../middlewares/authMiddleware'

const workersRoute = express.Router();

workersRoute.post("/create-user", userAuthorization(["Admin"]), bodyValidation(createUserSchema), isUsernameAlreadyExist, isWorkerAlreadyExists, workerscontroller.createUserController);
...
workersRoute.put("/update-user", userAuthorization(["Admin"]), bodyValidation(updateUserSchema), isUsernameAlreadyExist, isUserExistsById,workerscontroller.updateUser);
